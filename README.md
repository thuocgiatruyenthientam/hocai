...
Environment variables can be added via `.env.local` for per-environment configuration if needed. Update the `siteUrl` constants in `src/app/layout.tsx`, `src/app/sitemap.ts`, `src/app/rss/route.ts`, and `src/app/robots.ts` when deploying to production.
